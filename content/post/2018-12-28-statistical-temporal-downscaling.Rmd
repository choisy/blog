...
define **smoothness** as the sum of the absolute differences between the slopes
of adjacent linear models. The following function computes such a statistic from
a list of linear models:

```{r}
smoothness <- function(list_of_models) {
  require(magrittr)
  list_of_models %>% 
    sapply(get_slope) %>%
    diff() %>% 
...
`smoothness()` in order to find the value of smoothness that corresponds to our
choice of `y2_0`. And the combination of `make_models()` and `smoothness()` can
be used by the base R `optimize()` function in order to find the optimal value
of `y2_0` over a given interval that minimize the global smoothness. The problem
now is to define the interval to search in. An option could be given by the
following function:

```{r}
make_interval <- function(x) {
  the_range <- range(x)
  c(-1, 1) * diff(the_range) + the_range
}
```

that basically triples the range of a variable equally each side of its current
range. We can now look for the optimal value of `y2_0` by the following call:

```{r eval = FALSE}
optimize(function(x) smoothness(make_models(x)), make_interval(y))$minimum
```
...
unlist(Map(predict,
           models,
           lapply(make_new_x(x, n), function(x) data.frame(x_val = x))))
```
